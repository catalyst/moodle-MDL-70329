...
function xmldb_main_upgrade($oldversion) {
    ...
    if ($oldversion < 2021052500.82) {
        ...
        // Define key, index and fields to be dropped from question.
        $table = new xmldb_table('question');

        $index = new xmldb_index('categoryidnumber', XMLDB_INDEX_NOTUNIQUE, ['category', 'idnumber']);
        ...
        // Launch drop key modifiedby.
        $key = new xmldb_key('modifiedby', XMLDB_KEY_FOREIGN, ['modifiedby'], 'user', ['id']);
        $dbman->drop_key($table, $key);
        ...
        // Conditionally launch drop field stamp.
        $field = new xmldb_field('stamp');
        if ($dbman->field_exists($table, $field)) {
            $dbman->drop_field($table, $field);
        }
        ...
        // Conditionally launch rename field modifiedby.
        $field = new xmldb_field('modifiedby', XMLDB_TYPE_INTEGER, '10', null, null, null);
        if ($dbman->field_exists($table, $field)) {
            $dbman->rename_field($table, $field, 'modifierid');

            // Add a foreign key that was dropped before to rename.
            $table->add_key('modifierid', XMLDB_KEY_FOREIGN, ['modifierid'], 'user', ['id']);
        }

        // Main savepoint reached.
        upgrade_main_savepoint(true, 2021052500.82);
    }
