...
/**
 * Class core_course_moodleform_mod_test
 *
 * @package    core_course
 * @category   test
 * @copyright  2021 Catalyst IT Australia Pty Ltd
...
class moodleform_mod_test extends \advanced_testcase {
    ...
    public function test_moodleform_mod_features() {
        ...
        $assign = self::getDataGenerator()->create_module($modname, ['course' => $COURSE->id]);
        $assigncm = get_coursemodule_from_id($modname, $assign->cmid);
        $assign->instance = $assign->id;
        $assign->coursemodule = $assigncm;

        // Test availability feature enabled in moodleform_mod.
        $form = new mod_test_mod_form($assign, 0, $assigncm, $COURSE, $modname, true, false, false);
        $testavailability = '';
        ...
        $form = new mod_test_mod_form($assign, 0, $assigncm, $COURSE, $modname, false, false, false);
        $testavailability = '';
        foreach ((array)$form as $key => $formfeature) {
            if ($key === "\000*\000_form") {
                $testavailability = $formfeature;
        ...
        $form = new mod_test_mod_form($assign, 0, $assigncm, $COURSE, $modname, false, true, false);
        $testrestrictions = '';
        foreach ((array)$form as $key => $formfeature) {
            if ($key === "\000*\000_form") {
        ...
        $results = $testrestrictions->_elements[2]->_text;
        ...
        $form = new mod_test_mod_form($assign, 0, $assigncm, $COURSE, $modname, false, false, true);
        ...
        $results = $testcompletion->_elements[2]->_text;
