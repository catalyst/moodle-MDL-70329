...
class core_question_customfield_testcase extends advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        $this->question2data = [
                ...
                'customfield_f3' => $now, 'customfield_f4' => 1,
     ...
     */
    protected function restore_course($backupid, $fullname, $shortname, $categoryid) {
        global $CFG, $USER;
    ...
    public function test_create_question() {
        ...
        $handler->instance_form_save((object)$this->question2data);

        // Get the custom field data associated with these question ids.
        $q1cfdata = $handler->export_instance_data_object($question1->id);
        $q2cfdata = $handler->export_instance_data_object($question2->id);

        $this->assertEquals('some text', $q1cfdata->f1);
        $this->assertEquals('Yes', $q1cfdata->f2);
        $this->assertEquals(userdate(1632278491, get_string('strftimedaydatetime')), $q1cfdata->f3);
        ...
        $this->assertEquals(userdate(1632278491, get_string('strftimedaydatetime')), $q2cfdata->f3);
    ...
    public function test_backup_restore() {
        ...
        $newquestion1 = $DB->get_record('question', array('idnumber' => 'q1'));
        $newquestion1cfdata = $handler->export_instance_data_object($newquestion1->id);
        $this->assertEquals('some text', $newquestion1cfdata->f1);
        $this->assertEquals('Yes', $newquestion1cfdata->f2);
        $this->assertEquals(userdate(1632278491, get_string('strftimedaydatetime')), $newquestion1cfdata->f3);
        $this->assertEquals('b', $newquestion1cfdata->f4);
        $this->assertEquals('test', $newquestion1cfdata->f5);

        $newquestion2 = $DB->get_record('question', array('idnumber' => 'q2'));
        $newquestion2cfdata = $handler->export_instance_data_object($newquestion2->id);
        $this->assertEquals('some more text', $newquestion2cfdata->f1);
        $this->assertEquals('No', $newquestion2cfdata->f2);
        $this->assertEquals(userdate(1632278491, get_string('strftimedaydatetime')), $newquestion2cfdata->f3);
