...
if ($id) {
    ...
    if ($makecopy) {
        ...
        $question->idnumber = isset($question->idnumber) ? core_question_find_next_unused_idnumber($question->idnumber, $category->id) : '';
...
if ($mform->is_cancelled()) {
    ...
    if ($makecopy) {
        $question->id = 0;
        $question->status = \core_question\local\bank\question_version_status::QUESTION_STATUS_READY; // Copies should not be hidden.
    }
