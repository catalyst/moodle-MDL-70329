 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbank_tagquestion_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_submit_tags_form_incorrect_context_id() {
        ...
        // We should receive an exception if the question doesn't exist.
        $this->expectException('moodle_exception');
        \qbank_tagquestion\external\qbank_tagquestion_external::submit_tags_form($question->id, $missingcontextid, $formdata);
    }
    ...
    public function test_submit_tags_form_tags_disabled() {
        ...
        $this->setUser($user);
        $CFG->usetags = false;
        $result = \qbank_tagquestion\external\qbank_tagquestion_external::submit_tags_form(
                $question->id, $editingcontext->id, $formdata);
        $CFG->usetags = true;
