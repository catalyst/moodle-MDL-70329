...
class helper {
     ...
     * @return array
     */
    public static function get_context_and_heading(question_category_object $qcobject): string {
        global $PAGE, $OUTPUT;
        $data = [];
    ...
    public static function create_category_action_menu($cmid, $categoryid, $contextid) : string {
        global $OUTPUT, $PAGE;
        ...
        //if ($item->children->editable) {
            ...
            if (!self::question_is_only_child_of_top_category_in_context($categoryid)) {
                // Sets up delete link.
                $deleteurl = new moodle_url('/question/bank/managecategories/category.php',
                    ['cmid' => $cmid, 'delete' => $categoryid, 'sesskey' => sesskey()]);
                $menu->add(new action_menu_link(
                    $deleteurl,
        ...
        $exporturl = new moodle_url('/question/export.php',
            ['cmid' => $cmid, 'cat' => $categoryid . ',' . $contextid]);
        $menu->add(new action_menu_link(
            $exporturl,
    ...
    public static function create_category_questionbankurl($cmid, $categoryid, $contextid) : string {
        $questionbankurl = new moodle_url('/question/edit.php');
        $questionbankurl->param('cmid', $cmid);
        $questionbankurl->param('cat', $categoryid . ',' . $contextid);
        return $questionbankurl->__toString();
    }

    public static function create_category_handle($categoryid) : string {
        global $OUTPUT;

        if (!self::question_is_only_child_of_top_category_in_context($categoryid)) {
            return $OUTPUT->pix_icon('grip-vertical-solid', 'gripvsol', 'qbank_managecategories');
        } else {
            return $handle = '';
        }
    }
