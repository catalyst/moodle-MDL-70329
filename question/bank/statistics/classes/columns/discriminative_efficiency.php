...
class discriminative_efficiency extends column_base {
    ...
    protected function display_content($question, $rowclasses): void {
        global $PAGE;
        // Get all quizzes that use the question.
        $quizzes = helper::get_quizzes($question->id);
        $attributes = [];
        if (empty($quizzes)) {
            echo \html_writer::tag('div',
                get_string('na', 'qbank_statistics'),
                $attributes);
        } else {
            // Average discriminative efficiency per quiz.
            $discriminativeefficiency = helper::calculate_average_question_discriminative_efficiency($quizzes, $question->id);
