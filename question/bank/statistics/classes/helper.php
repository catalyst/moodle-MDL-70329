...
class helper {
     ...
     *
     * @param stdClass|int $quizorid quiz object or its id
     * @return all_calculated_for_qubaid_condition
     */
    private static function load_question_stats($quizorid): all_calculated_for_qubaid_condition {
        $quiz = $quizorid;
        if (!is_object($quiz)) {
            // Does not need to do a DB call here as we only need quiz ID.
            $quiz = new \stdClass();
            $quiz->id = $quizorid;
        }
     ...
     * @return float|int
     */
    public static function load_question_stats_item($quiz, $questionid, $item): ?float {
     ...
     * @return float|null
     */
    private static function calculate_average_question_stats_item($questionid, $item): ?float {
        $quizzes = self::get_quizzes($questionid);

        $sum = 0;
        $quizcount = count($quizzes);
        foreach ($quizzes as $quiz) {
            $value = self::load_question_stats_item($quiz, $questionid, $item);
            if (!is_null($value)) {
                $sum += $value;
            } else {
     ...
     * @return float|null
     */
    public static function calculate_average_question_discrimination_index($questionid): ?float {
    ...
    /**
     * Add classes based stats value.
     *
     * @param float|null $value stats value
     * @return string
     */
    public static function add_css_classes(?float $value): string {
        if (is_null($value) || $value < 30) {
