...
class helper_test extends \advanced_testcase {
     ...
     */
    public function test_get_quzziess() {
        global $DB;
        $this->resetAfterTest();
        // Create a course.
        ...
        $quiz1 = $generator->create_instance(array('course' => $course->id, 'timecreated' => 0));
        $quiz2 = $generator->create_instance(array('course' => $course->id, 'timecreated' => 0));
        $quiz3 = $generator->create_instance(array('course' => $course->id, 'timecreated' => 0));
        ...
        // Add question 1 to quiz 1, 2, and 3.
        quiz_add_quiz_question($question1->id, $quiz1);
        quiz_add_quiz_question($question1->id, $quiz2);
        quiz_add_quiz_question($question1->id, $quiz3);

        // Add question 2 to quiz 1 and 3.
        quiz_add_quiz_question($question2->id, $quiz1);
        quiz_add_quiz_question($question2->id, $quiz3);

        // Checking quizzes that contain question 1.
        $question1quizzes = helper::get_quizzes($question1->id);
        $this->assertCount(3, $question1quizzes);
        $this->assertArrayHasKey($quiz1->id, $question1quizzes);
        $this->assertArrayHasKey($quiz2->id, $question1quizzes);
        $this->assertArrayHasKey($quiz3->id, $question1quizzes);

        // Checking quizzes that contain question 2.
        $question2quizzes = helper::get_quizzes($question2->id);
        $this->assertCount(2, $question2quizzes);
        $this->assertArrayHasKey($quiz1->id, $question2quizzes);
        $this->assertArrayHasKey($quiz3->id, $question2quizzes);
    }
     ...
     * @throws \coding_exception
     */
    private function prepare_quizzes() {
        // Create a course.
        $course = $this->getDataGenerator()->create_course();
     ...
     */
    private function submit_quiz($quiz, $answers) {
        // Create user.
    ...
    /**
     * @param $correctanswerflags array of 1 or 0
     * 1 : generate correct answer
     * 0 : generate wrong answer
     * @return array
     */
    private function generate_attempt_answers($correctanswerflags) {
        $attempt = [];
        for ($i = 1; $i <= 4; $i++) {
     ...
     * @return \Generator
     */
    public function load_question_facility_provider() {
     ...
     */
    public function test_load_question_facility($quiz1attempts, $expectedquiz1facilities,
                                                $quiz2attempts, $expectedquiz2facilities,
        ...
        // Quiz 1 facilities.
        $quiz1facility1 = helper::load_question_facility($quiz1, $questions[1]->id);
        $quiz1facility2 = helper::load_question_facility($quiz1, $questions[2]->id);
        $quiz1facility3 = helper::load_question_facility($quiz1, $questions[3]->id);
        $quiz1facility4 = helper::load_question_facility($quiz1, $questions[4]->id);
        ...
        // Quiz 2 facilities.
        $quiz2facility1 = helper::load_question_facility($quiz2, $questions[1]->id);
        $quiz2facility2 = helper::load_question_facility($quiz2, $questions[2]->id);
        $quiz2facility3 = helper::load_question_facility($quiz2, $questions[3]->id);
        $quiz2facility4 = helper::load_question_facility($quiz2, $questions[4]->id);
        ...
        $quizzes = [$quiz1, $quiz2];
        $averagefacility1 = helper::calculate_average_question_facility($quizzes, $questions[1]->id);
        $averagefacility2 = helper::calculate_average_question_facility($quizzes, $questions[2]->id);
        $averagefacility3 = helper::calculate_average_question_facility($quizzes, $questions[3]->id);
        $averagefacility4 = helper::calculate_average_question_facility($quizzes, $questions[4]->id);
     ...
     * @return \Generator
     */
    public function test_load_question_discriminative_efficiency_provider() {
        yield 'Discriminative efficiency case 1' => [
            'Quiz 1 attempts' => [
                $this->generate_attempt_answers([1, 0, 0, 0]),
                $this->generate_attempt_answers([1, 1, 0, 0]),
            ...
            'Expected quiz 1 discriminative efficiency' => ['0.00%', '33.33%', '33.33%', '100.00%'],
            'Quiz 2 attempts' => [
                $this->generate_attempt_answers([1, 1, 1, 1]),
            ...
            'Expected average efficiency' => ['25.00%', '41.67%', '41.67%', '75.00%'],
        ];
    }

    /**
     * Test question discriminative efficiency.
     *
     * @dataProvider test_load_question_discriminative_efficiency_provider
     */
    public function test_load_question_discriminative_efficiency($quiz1attempts, $expectedquiz1discriminativeefficiency,
                                                                 $quiz2attempts, $expectedquiz2discriminativeefficiency,
                                                                $expectedaveragediscriminativeefficiency) {
        $this->resetAfterTest();
        ...
        $discriminativeefficiency1 = helper::load_question_discriminative_efficiency($quiz1, $questions[1]->id);
        $discriminativeefficiency2 = helper::load_question_discriminative_efficiency($quiz1, $questions[2]->id);
        $discriminativeefficiency3 = helper::load_question_discriminative_efficiency($quiz1, $questions[3]->id);
        $discriminativeefficiency4 = helper::load_question_discriminative_efficiency($quiz1, $questions[4]->id);
        ...
        $discriminativeefficiency1 = helper::load_question_discriminative_efficiency($quiz2, $questions[1]->id);
        $discriminativeefficiency2 = helper::load_question_discriminative_efficiency($quiz2, $questions[2]->id);
        $discriminativeefficiency3 = helper::load_question_discriminative_efficiency($quiz2, $questions[3]->id);
        $discriminativeefficiency4 = helper::load_question_discriminative_efficiency($quiz2, $questions[4]->id);
        ...
        $quizzes = [$quiz1, $quiz2];
        $avgdiscriminativeefficiency1 = helper::calculate_average_question_discriminative_efficiency($quizzes, $questions[1]->id);
        $avgdiscriminativeefficiency2 = helper::calculate_average_question_discriminative_efficiency($quizzes, $questions[2]->id);
        $avgdiscriminativeefficiency3 = helper::calculate_average_question_discriminative_efficiency($quizzes, $questions[3]->id);
        $avgdiscriminativeefficiency4 = helper::calculate_average_question_discriminative_efficiency($quizzes, $questions[4]->id);
